...
pub struct Filter {
    ...
    range_filter_rules: Option<Rules>,
}
...
impl Filter {
    pub fn new(
        ...
        let ground_filter_range = Some(config.filter.ground_filter_range);
        let lidar_distance_bound = Some(Bound::new(config.filter.lidar_distance_bound.imin,  config.filter.lidar_distance_bound.imax ));
        let range_filter_rules = Some(config.filter.rules.clone());
        ...
        Self {
            ...
            range_filter_rules: range_filter_rules,
        }
    }
